...
export default function Login({ password: passwordForm, oidc: oidcForm, messages }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  const { user } = useAuth();
    ...
    <GuestLayout>
      ...
      <div>
        ...
        <CardContent>
          ...
          <Typography color="textSecondary">
            No account?{' '}
            <NextLink href="/auth/registration" passHref>
              <Link>Register</Link>
            </NextLink>
          </Typography>
        </CardContent>
